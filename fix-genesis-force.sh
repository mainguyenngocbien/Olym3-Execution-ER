...
# Fix Genesis Force - Force Custom Genesis with Disabled Checkpoint Sync
# This script will force Erigon to use our custom genesis without checkpoint sync

set -e

echo "🔧 Fix Genesis Force - Force Custom Genesis"
...
rm -rf /root/.local/share/erigon/logs
...
print_header "Creating Genesis Force Service File"
...
Environment=XDG_CONFIG_HOME=/root/erigon_data
...
print_status "Genesis force service file created"

print_header "Creating Custom Config Directory"
mkdir -p /root/erigon_data/config
cat > /root/erigon_data/config/erigon.toml << 'EOF'
[chain]
chainId = 256000
networkId = 256000
...
print_status "Custom config created"
...
print_header "Creating Genesis Force Status Script"
...
print_header "Creating Genesis Force Configuration Summary"
cat > /root/olym3-summary.txt << EOF
=== Olym3 Testnet Season 3 Genesis Force Configuration ===
