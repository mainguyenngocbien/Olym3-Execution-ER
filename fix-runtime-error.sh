...
# Fix Runtime Error - Simple Erigon Configuration
# This script will fix the runtime panic error

set -e

echo "🔧 Fixing Runtime Error - Simple Configuration"
...
print_header "Testing Minimal Configuration"
print_status "Testing with only essential flags..."

# Test 1: Basic HTTP only
echo "Test 1: Basic HTTP configuration..."
timeout 10s ./erigon --datadir=/root/erigon_data --http --http.addr=0.0.0.0 --http.port=8545 --http.api=eth,net,web3 || echo "Test 1 failed"

sleep 2

# Test 2: With chain name
echo "Test 2: With chain name..."
timeout 10s ./erigon --datadir=/root/erigon_data --chain=olym3-testnet-s3 --http --http.addr=0.0.0.0 --http.port=8545 --http.api=eth,net,web3 || echo "Test 2 failed"

sleep 2

# Test 3: With port
echo "Test 3: With P2P port..."
timeout 10s ./erigon --datadir=/root/erigon_data --chain=olym3-testnet-s3 --http --http.addr=0.0.0.0 --http.port=8545 --http.api=eth,net,web3 --port=30303 || echo "Test 3 failed"
...
print_status "Service file created with minimal configuration"
